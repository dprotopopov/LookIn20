...
int main(int argc, char *argv[])
{
	...
	if (np>1) { // Рассылка исходных данных по процессам из ведушего процесса
		if (mp==0) {
			...
			buf.ddata[20] = tmax;
			buf.ddata[21] = epst;
			buf.idata[44] = n1; // количество ячеек сетки
			buf.idata[45] = n2; // количество ячеек сетки
			buf.idata[46] = ntp;
			buf.idata[47] = ntm;
			buf.idata[48] = lp;
		}
		MPI_Bcast(buf.ddata,100,MPI_DOUBLE,0,MPI_COMM_WORLD); // Брэдкаст начальных параметров
		if (mp>0) {
			...
			tmax = buf.ddata[20];
			epst = buf.ddata[21];
			n1   = buf.idata[44]; // количество ячеек сетки
			n2   = buf.idata[45]; // количество ячеек сетки
			ntp  = buf.idata[46];
			ntm  = buf.idata[47];
			lp   = buf.idata[48];
		}
